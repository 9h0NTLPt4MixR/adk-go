...
// turns by the workflow agent wrapper via session.State. The Input
// and PendingRequest.Payload fields are typed any and must
// therefore be JSON-encodable for the persisted state to round-
// trip. Nodes that need to carry binary data across a pause should
// store the bytes via agent.Artifacts and stash a URI string in
// place of the bytes, mirroring how the Live API surfaces audio.
