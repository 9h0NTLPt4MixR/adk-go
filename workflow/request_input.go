...
// WorkflowInputFunctionCallName is the magic FunctionCall name carried
// on a request event so the runner's generic ID-based dispatch
// (runner.findAgentToRun) can re-route the user's follow-up
// FunctionResponse back to the workflow agent that issued the
// request. Mirrors toolconfirmation.FunctionCallName.
...
// plus req.InterruptID in LongRunningToolIDs. This shape lets the
// runner's existing FunctionResponse-by-ID dispatch route the
// human's reply back to this workflow agent without any
// runner-side changes, and flips Event.IsFinalResponse() to true
// so the agent loop terminates after the request is yielded.
//
...
func NewRequestInputEvent(ctx agent.InvocationContext, req session.RequestInput) *session.Event {
	...
	// Synthesise the FunctionCall part so the runner's generic
	// FunctionResponse-by-ID dispatch finds this agent on the
	// follow-up turn. Args mirror the RequestInput fields so a
	// client that does not parse the dedicated RequestedInput
	// field can still display the prompt from the args alone.
	args := map[string]any{
