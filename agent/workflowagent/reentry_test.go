...
func TestWorkflowAgent_ReEntry_ResumesAtSameNode(t *testing.T) {
	...
	drainAgent(t, sess, a.Run(runCtx(sess, a, resumeMessage("decide", "approve"))), nil)
...
func TestWorkflowAgent_ReEntry_PreservesOriginalInput(t *testing.T) {
	...
	drainAgent(t, sess, a.Run(runCtx(sess, a, &genai.Content{
		Parts: []*genai.Part{{Text: "draft"}},
	})), nil)
...
func TestWorkflowAgent_ReEntry_NoSuccessorBeforeOutput(t *testing.T) {
	...
	drainAgent(t, sess, a.Run(runCtx(sess, a, resumeMessage("decide", "yes"))), nil)
